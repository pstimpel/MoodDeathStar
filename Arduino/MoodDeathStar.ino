...
void loop()
{
  ...
  int potiValue = analogRead(A7);//read poti, 0-1023  float voltage = sensorValue * (5.0 / 1023.0);

  brightness = map(potiValue, 0, 1023, 0, 255); //map the poti value to brightness
  if(brightness < 5) {
    brightness=0;
  }
  if(brightness > 250) {
  ...
  if( (oldbrightness < brightness-3) || (oldbrightness > brightness+3) || firstrun==true) {
    if(firstrun==true) {
      firstrun=false;
      Serial.println("first run correction after switching on");
    }
